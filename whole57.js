...
function windowSizeCheck() {
    if (window.innerWidth > 990) desktop = true;
...
var mainImageSketch = function (mainImageSketch) {
    ...
    const stringy = "Future Fit #3";
    let sizer = 92;
    const showText = true;
    const textAlpha = 255;
    const strokeAlpha = 20;
    const strokeColor = '#EFA89B';

    const fontSampleFactor = 0.02;
    ...
    let alrdycreatedcnvForIntro = false;
    ...
    function introSetup() {
        ...
        if (!desktop) {
            sizer = 46 * (mainImageSketch.width / mainImageSketch.height);
            cloth_width = 150;
        ...
        startingPoints = abrahamBlack.textToPoints(stringy, mainImageSketch.width / 8, mainImageSketch.height / 8 * 7, sizer, {
...
var soundImgSketch = function (soundsketch) {
    ...
    soundsketch.draw = function () {
        //fit for screens
        if (!mobile) {
            translateByX = soundsketch.width / 4;
            translateByY = soundsketch.height / 10 * 6;
...
var noisingSketch = function (etexS) {
    ...
    etexS.draw = function () {
        //fit for screens
        if (!mobile) {
            etexS.translate(-etexS.width / 4, 0, 0);
            etexS.scale(0.8)
        } else {
            etexS.scale(0.45);
...
var fallingAttractorsSketch = function (iotAttrct) {
    ...
    iotAttrct.setup = function () {
        ...
        num_particles = iotAttrct.width / 15;
        ...
        for (var i = 0; i < num_particles; i++) {
            ...
            for (let j = 0; j < 2; j++) {
                ...
                particles[i][j] = new Particle(new Vec2D(getRnd(0, iotAttrct.width - iotAttrct.width / 3), getRnd(-0.5 * iotAttrct.height, 0)));
    ...
    iotAttrct.mousePressed = function () {
        ...
        if (iotAttrct.winMouseX > 0 && iotAttrct.winMouseX < iotAttrct.width && iotAttrct.winMouseY > 0 && iotAttrct.winMouseY < iotAttrct.height) {
            ...
            mouseAttractor[mouseAttractor.length] = new AttractionBehavior(mousePos, iotAttrct.height, 3);
            physics.addBehavior(mouseAttractor[mouseAttractor.length - 1]);
        }
    }
    ...
    function releaser() {
        wasPressed = false;
        pressCounter = 0;
        for (let i = 0; i < mouseAttractor.length; i++) {
            physics.removeBehavior(mouseAttractor[i]);
            if (i == mouseAttractor.length - 1) {
                mouseAttractor = [];
            }
        }
    }
...
var etreeSketch = function (etreeskch) {
    ...
    function etreeSetup() {
        ...
        interStep = fr * step;
        if (desktop) goalRange = etreeskch.width / etreeskch.height * 10;
        else {
    ...
    etreeskch.draw = function () {
        if (mobile) {
            bptranslateby = etreeskch.height / 2;
            etreeskch.translate(0, bptranslateby, 0);
            etreeskch.scale(0.7);
        }
        ...
        if (etreeskch.winMouseX > 0 &&
            etreeskch.winMouseX < etreeskch.width &&
            etreeskch.winMouseY > 0 &&
            etreeskch.winMouseY < etreeskch.height) {
            if (etreeskch.winMouseX > etreeskch.width / 3 && desktop) mouseVec = etreeskch.createVector(etreeskch.width / 3, etreeskch.winMouseY);
            else mouseVec = etreeskch.createVector(etreeskch.winMouseX, etreeskch.winMouseY);
        }
    ...
    function updatePos(upVector) {
        if (etreeskch.frameCount % (fr * numOTrees) == 0) {
            etreeskch.clear();
            etreeskch.background(etreeskch.color('#251818'));
        }
...
var recraftEleSketch = function (elesketch) {
    ...
    elesketch.draw = function () {
        //fit for screens
        if (!mobile) elesketch.translate(-elesketch.width / 6, 0, 0);
        else {
            elesketch.scale(0.6);
...
var autodrawSketch = function (autosk) {
    ...
    autosk.draw = function () {
        if (!mobile) autosk.translate(-autosk.width / 6, -autosk.height / 6, 0);
        else {
...
var tdSketch = function (touchskch) {
    ...
    touchskch.draw = function () {
        ...
        if (!mobile) {
            touchskch.translate(-touchskch.width / 6, touchskch.height / 6, 0);
