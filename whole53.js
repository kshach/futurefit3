//Code and p5js sketches were written by Shachar Kantor, shachar.kantor@gmail.com
  ...
  soundsketch.draw = function() {
    //fit for screens
    if(soundsketch.width>500){translateByX = soundsketch.width/4;
  ...
  var noisingSketch = function(etexS) {
    ...
    etexS.draw = function() {
      //fit for screens
      if(etexS.width>500){ etexS.translate(-etexS.width/4,0,0);
      etexS.scale(0.8)
                }
  ...
  iotAttrct.setup = function() {
    ...
    document.addEventListener('iotevent', () => {
      ...
      num_particles = iotAttrct.width/20;
      // Set the world's bounding box (particles can't leave this box)
  ...
  etreeskch.draw = function() {
    ...
    if( etreeskch.winMouseX > 0 &&
      ...
      etreeskch.winMouseY < etreeskch.height) {
        if(etreeskch.winMouseX>etreeskch.width/3&&desktop)mouseVec = etreeskch.createVector(etreeskch.width/3, wetreeskch.inMouseY);
